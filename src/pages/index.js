...
export default function Home() {
  const {form, input, formHeader, emailIcon, headerWrapper, legend, sendEmailBtn, contactFieldSet, label, message} = styles
  ...
  function toggleModal(e){
      ...
      const showModalClassName = modal.getAttribute('data-showmodalclassname');
  ...
  return (
    <main>
      <Modal>
        <form className={form} action="https://getform.io/f/09976747-1aa0-41dc-9c30-912abb04af26" method="POST">
          <div className={headerWrapper}>
            <h2 className={formHeader}>Send A Message</h2>
            <img alt="email icon" className={emailIcon} src={EmailSVG}/>
          </div>
          <fieldset className={contactFieldSet}>
            <legend className={legend}>Your Information</legend>
            <label className={label}>
              Name
              <input className={input} required="required" type="text" name="name" />
            </label>
            <label className={label}>
              Email
            ...
            <label className={label}>
              Message
              <textarea className={message} required type="text" name="message" />
            </label>
          </fieldset>
